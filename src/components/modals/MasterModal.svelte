  ...
  async function handleSubmit() {
    ...
    try {
      if (type === 'status') {
        ...
        if (editId) {
          const updated = await updateMember(editId, { name: name.trim(), color, email: email.trim() });
          projects.update((list) => list.map((p) => p.id === proj.id ? { ...p, members: p.members.map((m) => m.id === updated.id ? updated : m) } : p));
        } else {
          const created = await createMember(proj.id, name.trim(), email.trim(), color);
  ...
  >
    <div class="bg-white rounded-2xl shadow-[var(--shadow-modal)] w-full max-w-sm p-6 flex flex-col gap-5">
      ...
      <div class="flex flex-col gap-3">
        ...
        {#if type === 'member'}
          <div class="flex flex-col gap-1">
            <label class="text-xs font-medium text-[var(--ink-soft)]" for="ms-email">Email</label>
            <input
              id="ms-email"
              type="email"
